...
def should_use_adaptive_rag(user_input: str) -> bool:
    """
    In Adaptive RAG mode, route ALL research-intent queries through the RAG
    workflow so retrieved papers always ground the answer.
    Only skip RAG for PDF requests, image requests, and combined draft+pdf
    requests (those are handled by their own branches upstream).
    """
    if st.session_state.agent_mode != "adaptive_rag":
        return False
    # PDF and image requests have dedicated branches — don't double-handle
    if looks_like_pdf_request(user_input):
        return False
    if looks_like_draft_request(user_input):
        return False
    return True
...
def format_adaptive_rag_response(result: dict) -> str:
    """
    Produce a clean, conversational output from the adaptive RAG result.
    The LLM-synthesised final_answer is the centrepiece; metadata is shown
    compactly below it without overwhelming the reader.
    """
    final_answer    = (result.get("final_answer") or "").strip()
    confidence_note = (result.get("confidence_note") or "").strip()
    relevant_docs   = result.get("relevant_documents", []) or []
    ...
    if final_answer:
        ...
        parts.append(
            "I wasn't able to find strongly relevant papers for that query. "
            "Try rephrasing with more specific keywords, or switch to Standard Research mode."
        )

    # ── 2. Confidence badge (subtle) ────────────────────────────────────────
    if confidence_note:
        parts.append(f"\n*{confidence_note}*")

    # ── 3. Sources panel ────────────────────────────────────────────────────
    docs_to_show = relevant_docs[:5] if relevant_docs else retrieved_docs[:5]
    if docs_to_show:
        parts.append("\n---\n**📄 Sources retrieved**")
    ...
    # ── 4. Error note (if any) ──────────────────────────────────────────────
    if retrieval_error:
        parts.append(f"\n⚠️ *Retrieval note: {retrieval_error}*")
