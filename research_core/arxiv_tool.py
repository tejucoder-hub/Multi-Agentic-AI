...
# ── Cache ─────────────────────────────────────────────────────────────────────
_cache: dict = {}
_CACHE_TTL = 10  # minutes


def _cache_get(key: str) -> Optional[list]:
    if key in _cache:
        docs, ts = _cache[key]
        if datetime.now() - ts < timedelta(minutes=_CACHE_TTL):
            print(f"[arxiv_tool] Cache hit: '{key}'")
            return docs
        del _cache[key]
    return None


def _cache_set(key: str, docs: list):
    _cache[key] = (docs, datetime.now())
...
_DISCOVERY_STOPWORDS = {
    ...
    "papers", "paper", "research", "about", "on", "survey", "some",
    "top", "best", "articles", "article", "in", "the", "of", "and",
    "or", "for", "with", "what", "is", "are", "tell", "explain",
    "how", "does", "do", "a", "an",
}
...
_ABBREVIATIONS = {
    ...
    " rlhf ":      " reinforcement learning from human feedback ",
    " mcp ":       " model context protocol ",
    " agentic ai ": " agentic ai autonomous agents ",
    " agents ":    " autonomous agents large language model agents ",
...
def normalise_topic(topic: str) -> str:
    ...
    # Strip discovery words
    tokens = topic.lower().strip().split()
    tokens = [t.strip(".,?!;:") for t in tokens
              if t.strip(".,?!;:") not in _DISCOVERY_STOPWORDS
              and len(t.strip(".,?!;:")) > 1]
    text = f" {' '.join(tokens)} "
...
def extract_query_terms(topic: str) -> List[str]:
    ...
    stopwords = {
        "the", "a", "an", "of", "on", "for", "to", "in", "and", "or",
        "latest", "recent", "paper", "papers", "research",
    }
    return list(dict.fromkeys(
        w for w in cleaned.split() if w not in stopwords and len(w) > 2
    ))


def build_arxiv_query(topic: str) -> str:
    return "+".join(normalise_topic(topic).split())


def parse_year(date_text: str):
    if not date_text:
        return None
    try:
        return datetime.strptime(date_text, "%Y-%m-%dT%H:%M:%SZ").year
...
# ── Fetch via arxiv library ───────────────────────────────────────────────────

def _fetch_with_arxiv_lib(topic: str, max_results: int = 10) -> dict:
    import arxiv
...
# ── Fetch via raw requests (fallback) ─────────────────────────────────────────

def _fetch_with_requests(topic: str, max_results: int = 10) -> dict:
    query   = build_arxiv_query(topic)
    url     = (
    ...
    for attempt, delay in enumerate(delays, 1):
        try:
            ...
            if resp.status_code == 429:
                wait = delay * 4
                print(f"[arxiv_tool] 429 rate limit. Waiting {wait}s (attempt {attempt}/3)...")
                time.sleep(wait)
                continue
            ...
            print(f"[arxiv_tool] Error attempt {attempt}/3: {e}. Waiting {delay*2}s...")
            time.sleep(delay * 2)
            continue

    raise RuntimeError("arXiv unavailable after 3 attempts. Please try again shortly.")


def _parse_arxiv_xml(xml_content: str) -> dict:
    ns = {
        "atom":  "http://www.w3.org/2005/Atom",
        "arxiv": "http://arxiv.org/schemas/atom",
    }
    ...
    for entry in root.findall("atom:entry", ns):
        ...
        pdf_link = primary = ft("id")
        for link in entry.findall("atom:link", ns):
            href = link.attrib.get("href", "")
            if link.attrib.get("rel") == "alternate" and href:
                primary = href
            if (link.attrib.get("type") == "application/pdf" or
                    link.attrib.get("title") == "pdf") and href:
                pdf_link = href

        pub = ft("published")
        entries.append({
            "title":      ft("title"),
            "summary":    ft("summary")[:1500],
            "authors":    authors,
            "categories": categories,
            "published":  pub,
            "updated":    ft("updated"),
...
def compute_relevance_score(paper: Dict, query_terms: List[str]) -> float:
    ...
    for term in query_terms:
        ...
        if term in summary:    score += 2.0
        if term in categories: score += 1.5

    phrase = " ".join(query_terms)
    if phrase and phrase in title:   score += 4.0
    if phrase and phrase in summary: score += 2.0

    agent_terms = {"agentic", "agents", "autonomous", "multi", "agent"}
    if any(t in query_terms for t in agent_terms):
        if "agent" in title or "agents" in title:     score += 4.0
        if "agent" in summary or "agents" in summary: score += 2.0

    current_year = datetime.now().year
    if year:
        if year >= current_year - 1:   score += 4.0
        elif year >= current_year - 2: score += 3.0
        elif year >= current_year - 3: score += 2.0
        elif year >= current_year - 5: score += 1.0
...
def rerank_papers(topic: str, entries: List[Dict]) -> List[Dict]:
    ...
    for paper in entries:
        p = dict(paper)
        p["relevance_score"] = round(compute_relevance_score(paper, query_terms), 2)
        ranked.append(p)

    ranked.sort(
        key=lambda x: (x.get("relevance_score", 0), x.get("year") or 0),
        reverse=True,
    )
    return ranked
...
def arxiv_search(topic: str) -> list[dict]:
    """
    Search arXiv for recent, relevant research papers on a given topic.

    Args:
        topic: Core research topic (e.g. "retrieval augmented generation",
               "vision transformer").  Do NOT include words like "latest",
               "find", "papers on" — pass only the topic.

    Returns:
        Ranked list of up to 5 paper dicts with title, summary, authors,
        year, link, pdf link, and relevance_score.
    """
    cache_key = normalise_topic(topic)
    cached    = _cache_get(cache_key)
    if cached is not None:
        return cached

    # Try arxiv library first (handles rate limiting gracefully)
    try:
        raw = _fetch_with_arxiv_lib(topic, max_results=10)
    except Exception as e:
        print(f"[arxiv_tool] arxiv-lib failed ({e}), falling back to requests...")
        raw = _fetch_with_requests(topic, max_results=10)
    ...
    if not entries:
        return [{
            "title":           "No papers found",
            "summary":         f"No arXiv papers found for: {topic}",
            "authors":         [],
            "categories":      [],
            "published":       "",
            "updated":         "",
    ...
    ranked = rerank_papers(topic, entries)[:5]
